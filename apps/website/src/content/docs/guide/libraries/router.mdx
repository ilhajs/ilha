...
```ts
input.load; // { loading: boolean, value: T | undefined, error: Error | undefined }
```

The result also keeps its legacy flat spread into `input`, so existing pages are unaffected. Server `load` data never carries an envelope — it always settles before render, so there is nothing to track.
