...
Registers a route. Uses [rou3](https://github.com/h3js/rou3) for matching. First match wins.
...
- A page can export **both**: `load` runs during SSR (first paint), `clientLoad` runs on client navigations instead of the endpoint fetch. Make them return the same shape, then infer the page input from either loader.
