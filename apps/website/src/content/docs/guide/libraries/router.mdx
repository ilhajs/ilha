...
<PackageManagers pkg="@ilha/router" />
...
| `@ilha/router/server-island-registry` | `setFrameGuard()` — gate frame requests.                                             |
...
Details: [Server islands](#server-islands-servertsx), [Server pages](#server-pages-fooservertsx).
...
The builder also exposes `render(url)`, `renderHydratable(url, registry, options?, request?)`, and `renderResponse(...)` for hosting setups that render routes in their own fetch handler instead of the built-in middleware. You do not need them for file-system routing with [oxidejs](https://npmjs.com/package/oxidejs) — SSR is already embedded.
...
Throws a loader error sentinel. On the server, intercept it with `.renderResponse()` (custom hosts) or the SSR middleware, which emits the proper HTTP status code automatically.
...
export default ilha<InferLoader<typeof load>>(({ input }) => (
  <Dashboard stats={input.stats} />
));
```

#### The `input.load` envelope

Client-loader results are exposed through an envelope shaped like ilha's derived one — call it to read the value, or read `loading`/`value`/`error` directly:

```ts
input.load(); // → { stats } — same as input.load.value
input.load.loading; // false once the loader has settled
input.load.error; // undefined when no error
```

Server `load` data uses the **same envelope** — every loader result, server or client, reads through `input.load`. During server rendering it is a plain object (`input.load.value.stats`); on the client it is a callable accessor (`input.load()`).
...
:::note
Frame requests are unauthenticated by default — anyone can re-render any server island by id. If an island's state holds private data, gate frames.
:::
...
import { setFrameGuard } from "@ilha/router/server-island-registry";
...
:::note
Forwarding closures (`onclick={() => action.remove(id)}`) are invoked once per render to capture the action call. Keep them pure: a closure that only calls one action. Side effects around the call run on the server during every frame.
:::
...
During client navigation to a server page the view host starts empty until the first frame lands — pair it with a pending indicator (see [Transitions](#transitions)).
...
Server islands and server pages pull their markup through `/__ilha/frame`; regular-page loaders fetch their data from `/__ilha/loader`. Both endpoints are served by the [`"@ilha/router/ssr"` middleware](#server-side-rendering) — no host code required.
...
| Form submit handlers       | [Store — Forms](/guide/libraries/store#forms) |
| Route context signals      | [Signals](/guide/helpers/signals)             |
| Hydratable SSR             | [Hydratable](/guide/island/hydratable)        |
| Astro integration          | [Astro](/guide/libraries/astro)               |
