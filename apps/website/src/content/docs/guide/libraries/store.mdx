...
<PackageManagers pkg="@ilha/store" />
...
| `@ilha/store/form`    | Form extraction, validation, issue-to-error mapping                                                                        |
...
For cache, dedup, and cross-store sharing of the same fetch, wrap the work in [`query()`](#query--data-fetching-cache) from `@ilha/store/query`.
...
`persist`'s sibling for the query string, from the `@ilha/store/persist` entry point. Instead of one serialized blob, each state key gets **its own search param** (`?q=boots&page=2`) — shareable, reload-safe, back/forward-friendly. Prior art: [nuqs](https://nuqs.dev) for React.

The canonical use case is a filter bar. UI state that drives data loading (search text, sort, page) belongs in the URL, because `@ilha/router` loaders re-run on every navigation and read `ctx.url.searchParams`. The cycle is **one-way**: store → URL → loader → render. The store is the _write_ path for the controls; the loader stays the _read_ path for data — on the server and the client. Because same-island navigations [update the mounted island in place](/guide/libraries/router#loaders) (no remount), the bound input keeps focus, caret, and scroll while the loader-driven parts of the page refresh.
...
**Rule of thumb:** if the data is addressable by URL and should survive reload/back-forward, use `persist` / `persistQuery` (`@ilha/store/persist`) plus a loader. If the data is cross-island shared state with no URL semantics, use [`query()`](#query--data-fetching-cache) inside a `.derived()`.

## `query()` — data-fetching cache

`@ilha/store/query` adds cache, deduplication, cross-store sharing, and invalidation for async `.derived()` fetches — on both store builders and island [`.derived()`](/guide/island/derived#cache-fetches-with-ilhastorequery). The host envelope is unchanged.
...
<Aside type="caution" title="SSR and module-level stores">
...
For request-scoped data, prefer `@ilha/router` loaders and feed results into the store via `hydrate()` on the client. `query()` fits client-only stores (create after mount, dispose on unmount), or module singletons whose key comes from client-only reactive state. Pass a fresh `QueryCache` via `query({ cache })` when you need request-isolated server caching.
