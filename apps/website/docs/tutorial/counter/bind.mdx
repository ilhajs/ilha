...
The `bind:` prefix goes on the attribute, and the value is a signal accessor — either local `state()` or an external `signal()`. Ilha infers the correct value property automatically: `value` for text and number inputs, `checked` for checkboxes.
