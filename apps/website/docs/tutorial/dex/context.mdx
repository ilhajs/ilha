...
export const script = dedent`
    ...
    const PokemonPicker = ilha
      ...
      .render(({ state }) => {
        ...
        return <>
          ...
          <select id="pokemon" bind:value={pokemon}>
            {options}
          </select>
        </>;
      });
    ...
    const PokemonStats = ilha
      ...
      .render(({ input }) => {
        const stats = input.stats.map(({ key, value }) => <tr>
          <td>{key}</td>
          <td>{value}</td>
        </tr>);
        return <div class="table" style="width:100%">
          <table>
            <thead>
              <tr>
                <th>Stat</th>
    ...
    const PokemonCard = ilha
      ...
      .render(({ derived }) => {
        if (derived.pokemonData.loading) return <p>Loading Pokémon...</p>;
        if (derived.pokemonData.error) return <p>{derived.pokemonData.error.message}</p>;

        const { name, sprites, types, stats } = derived.pokemonData()!;
        const typesBadges = types.map(({ type }) => <span class="badge">{type.name}</span>);
        ...
        return <>
          ...
          <h2>{name}</h2>
          {typesBadges}
          <PokemonStats stats={mappedStats} />
        </>;
      });

    export const Example = ilha.render(() => <>
      <PokemonPicker />
      <PokemonCard />
    </>);
...
example above, `PokemonPicker` writes to the context via `bind:value=`, and
