...
export const script = dedent`
    ...
    const PokemonPicker = ilha
      ...
      .render(({ state }) => {
        ...
        const options = state.pokemonList().map(
          ({ name }) => <option value={name} selected={name === currentPokemon}>{name}</option>
        );
    ...
    export const Example = ilha.render(() => <PokemonPicker defaultPokemon="charizard" />);
...
render a child island directly in JSX:

```ts
const Pokedex = ilha.render(() => <PokemonPicker defaultPokemon="charizard" />);
```
