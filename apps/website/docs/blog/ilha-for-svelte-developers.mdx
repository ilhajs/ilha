...
Svelte’s `$state` lets you read and write a reactive variable directly. Ilha’s `state()` returns a signal accessor: call it with no argument to read, use `.set()` to replace the value, or `.update()` when the next value depends on the previous one.
...
| Update from current value | `count++`   | `count.update((value) => value + 1)` |
