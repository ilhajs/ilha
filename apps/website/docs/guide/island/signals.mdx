---
title: Signals
description: Create reactive signals and computed values, share state across islands, batch writes, and read values without creating dependencies.
---
...
Two parallel vocabularies exist on purpose: **island-local** primitives (`state()` / `derived()` / `action()` / `effect()` / `effect.once()` / `onError()`) live inside `ilha()` components, persist across rerenders, and are snapshot-restored on hydration. The **standalone** helpers above (`signal()` / `computed()` / `context()` / `batch()` / `untrack()` / `persist()` / module-scope `effect()`) live at module scope for shared state and one-off machinery. Calling `signal()` or `computed()` inside an island render creates a fresh value on every rerender — use `state()` and `derived()` there instead (a dev warning tells you when you do).
...
The updater form also works with `context()` and nested `.select()` accessors. If a signal stores a function, return the replacement function from an updater: `callback(() => nextCallback)`.

Reading the signal while an island renders, or inside `derived()` or `effect()`, automatically subscribes that scope, so when the signal changes, dependents re-run as if it were local state.
...
```tsx twoslash
import { action, ilha, signal } from "ilha";
...
Reading a computed while an island renders, inside `derived()`/`effect()`, or inside a standalone [`effect()`](#effectfn) subscribes that scope, so dependents re-run when the computed's resolved value changes:
...
```tsx twoslash
import { action, ilha, context } from "ilha";
...
```tsx twoslash
import { effect, ilha, context } from "ilha";
...
Import `effect` from `"ilha"` as a named export — `import { effect } from "ilha"`. Inside an island render, `effect()` registers an island effect slot; outside an island, it runs standalone. This page documents the standalone form. The top-level `effect()` documented here is a standalone helper for code that runs outside any island.
...
Multiple synchronous signal writes inside the effect body produce a single propagation pass, the same as inside `effect()`:
...
```tsx twoslash
import { effect, ilha, signal, untrack } from "ilha";
