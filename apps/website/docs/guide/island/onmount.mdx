...
`.onMount()` is **client-only** — SSR never invokes it (matching `.on()` and `.effect()`), so server-rendered markup must not depend on onMount side effects. Use it to seed client-side state on hydration instead.
...
`unmount()` detaches the island's listeners before it calls the cleanup function returned by `.onMount()`.

For abortable APIs, pass the mount signal instead of returning manual cleanup:
...
The signal aborts when the island unmounts.
