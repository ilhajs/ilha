---
title: .onError()
description: Per-island .onError() handlers and app-wide onUncaughtError() for errors from events, effects, actions, mounting, and transitions.
---

import { example } from "./onerror.examples";
...
- [`.on()`](/guide/island/action#use-on-for-advanced-listeners) — sync throws and async rejections (except filtered `AbortError`)
- [`.effect()`](/guide/island/effect) — sync throws from the effect body or its cleanup
- [`.action()`](/guide/island/action) — sync throws and async rejections, except cancellation
...
```ts
{
  ...
  source: "on" | "effect" | "action" | "mount" | "transition";
