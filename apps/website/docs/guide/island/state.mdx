...
State declared with `.state()` is local to one island. If you need to share a value across multiple islands, use [`context()`](/guide/island/signals) instead, which creates a named global signal.
