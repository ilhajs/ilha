...
const Hello = ilha(() => <p>Hello, ilha!</p>);
```
...
const Label = ilha<{ text: string }>(({ input }) => (
...
const Counter = ilha.state("count", 0).render(({ state }) => (
  <div>
    ...
    <button
      type="button"
      onclick={() => state.count((count) => count + 1)}
    >
      +
    </button>
  </div>
...
const Search = ilha(() => (
  <input
    oninput:abortable={async (event, { signal }) => {
...
Prefer native event props when one rendered element owns the event. Use [`.on()`](/guide/island/action#use-on-for-advanced-listeners) when you need a CSS selector, an island-host listener, the full island handler context, or multiple modifiers.
...
const Controls = ilha
  .state("count", 0)
  .on("[data-increment]@click:once:capture", ({ state }) => {
    state.count((count) => count + 1);
  })
  .render(({ state }) => (
    <button data-increment>Count: {state.count()}</button>
  ));
```
...
const Item = ilha
  ...
  .render(({ input, state, action }) => (
    <>
      {input.label}:{state.n()}
      <button type="button" onclick={action.bump}>
        +
      </button>
    </>
  ));
...
function ChangeButton({ change }: { change: () => void }) {
...
const Parent = ilha
  ...
  .render(({ state }) => (
    <section>
      <p>{state.value()}</p>
      <ChangeButton change={() => state.value("baz")} />
    </section>
  ));
```

Rendering a plain component outside an island produces static HTML, so event props stay inert. For independently mountable UI, promote it with [`ilha(() => JSX)`](#promote-it-to-an-island).
...
### Bind contracts for component authors
...
On initial mount, Ilha reflects bound properties before `.onMount()`, then attaches native bind listeners afterward. Controller listeners registered in `.onMount()` therefore observe user input before the bind write triggers reactive synchronization.

Register component operations with `.action()`. Use direct lowercase event props or a morph-safe [`.on()` selector](/guide/island/action#use-on-for-advanced-listeners) to call them.

Do not pass function handlers through attribute serializers or `raw()`. Those paths produce markup, not live event registrations.
...
If the island uses async [`.derived()`](/guide/island/derived) values, await the island to resolve all of them before rendering:
...
// Async SSR by name — same as `await Island()`
const html2 = await Island.toStringAsync();

// Sync — derived renders in loading state
const html3 = Island.toString();
```
...
island.toStringAsync(props?) // async SSR — always returns a Promise; awaits derived values
await island(props) // callable async form (also used for JSX/child-slot composition)
...
- Use lowercase native event props such as `onclick` and one optional modifier such as `oninput:abortable`; camelCase `onClick` props are omitted. Use [`.on()`](/guide/island/action#use-on-for-advanced-listeners) for host listeners, selectors, or combined modifiers.
