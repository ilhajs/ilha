...
Prefer native event props when one rendered element owns the event. Use [`.on()`](/guide/island/on) when you need a CSS selector, an island-host listener, the full island handler context, or multiple modifiers.
...
Two-way `bind:property={signal}` bindings synchronize form elements with signals. For the full contract — supported binding types, number coercion, `.select()` nested fields, radio and checkbox groups, element references, bind-prop contracts for component authors — see [Template bindings](/guide/island/bindings).
...
- Use lowercase native event props such as `onclick` and one optional modifier such as `oninput:abortable`; camelCase `onClick` props are omitted. Use [`.on()`](/guide/island/on) for host listeners, selectors, or combined modifiers.
