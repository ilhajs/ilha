...
const Save = ilha(() => {
  ...
  const bump = () => count.update((value) => value + 1);
...
`prefer-plain-handler` warns if `save` never reads a status field. Store a function in `state` with `.set`: `callback.set(nextFn)`.
