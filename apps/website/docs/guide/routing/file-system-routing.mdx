...
// Loader results arrive in a `load` envelope; InferLoader types the value.
type LoaderProps<T> = {
  load: {
    loading: boolean;
    value: T;
...
export default ilha<LoaderProps<InferLoader<typeof load>>>(
  ({ load }) => <h1>{load.value.user.name}</h1>,
);
```
...
type PageInput = {
  load: {
    loading: boolean;
    value: { stats: unknown } | undefined;
    error: Error | undefined;
  };
};
declare const input: PageInput;
