...
`@ilha/router` hardens SSR and server-island endpoints by default. This page is the full guard and origin model behind [Server islands](/guide/routing/server-islands).
...
| Rendering routes to a `Response` | [Server islands](/guide/routing/server-islands#use-a-custom-server-only-when-needed)      |
