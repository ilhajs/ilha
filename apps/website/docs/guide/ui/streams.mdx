...
function* Status() {
  yield Stream.map(Stream.fromIterable(["idle", "ready"]), (value) => (
    <p>{value}</p>
  ));
}
```
...
function* Search() {
  const query = atom("");
  yield (
    <input value={query} oninput={(e) => query.set(e.currentTarget.value)} />
  );
  yield* when(
...
function* Boot() {
  yield* Stream.runForEach(Stream.fromIterable([1, 2, 3]), (n) =>
