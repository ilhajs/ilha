...
| API                                               | Env    | Output                  |
| ------------------------------------------------- | ------ | ----------------------- |
| `await renderToString(setup)`                     | Server | HTML plus snapshot wrap |
| `await renderToString(setup, { markers: false })` | Server | Inner HTML only         |
| `mount(el, setup)`                                | Client | Live tree               |
| `mount(el, setup, { hydrate: true })`             | Client | Hydrate existing HTML   |
