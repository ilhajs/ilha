...
| `const View = function*` | You need `when`, `watch`, or `wait`           |
...
You write functions that return JSX. Generators `yield*` instructions. Async functions can await before they return a view.
