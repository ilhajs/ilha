...
If you prefer a lower-level option or need to avoid a build step, the [`html`` `](/guide/recipes/helpers#build-html-strings) tagged-template API is also available. It uses the same runtime and reactivity model, so you can mix both styles or migrate incrementally.
