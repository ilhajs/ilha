...
Ilha cannot register handlers hidden inside serialized attribute objects or `raw()` strings. Those paths produce plain markup. For broader composition, use [`.on()`](/guide/island/action#use-on-for-advanced-listeners) with a stable selector.

Use [`bind:*`](/guide/island/render#template-bindings) for signal synchronization. Use [`.on()`](/guide/island/action#use-on-for-advanced-listeners) for selectors, host listeners, its full handler context, or combined modifiers.
