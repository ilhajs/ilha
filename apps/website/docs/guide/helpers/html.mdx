...
Ilha cannot register handlers hidden inside serialized attribute objects or `raw()` strings. Those paths produce plain markup. For broader composition, use [`.on()`](/guide/island/on) with a stable selector.

Use [`bind:*`](/guide/island/bindings) for signal synchronization. Use [`.on()`](/guide/island/on) for selectors, host listeners, its full handler context, or combined modifiers.
