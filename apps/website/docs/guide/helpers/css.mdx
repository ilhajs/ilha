...
A common pattern is to use both together: author styles with css for tooling support, then pass the result to [`.css()`](/guide/island/css) to attach them:
...
const Card = ilha
  ...
  .render(({ state, action }) => (
    <div class="card">
      ...
      <button onclick={action.toggleExpanded}>
