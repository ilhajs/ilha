...
const CartButton = ilha
  .on("button@click", () => cartCount(cartCount() + 1))
  .render(() => html` <button>Add to cart</button> `);

const CartBadge = ilha.render(() => html` <span>${cartCount()}</span> `);
```
...
const SearchInput = ilha.bind("input", query).render(() => html`<input type="search" />`);
