...
Import `effect` from `"ilha"` as a named export — `import { effect } from "ilha"`. It is deliberately not exposed as `ilha.effect()`: the [island builder method](/guide/island/effect) of that name registers a per-island effect. The top-level `effect()` documented here is a standalone helper for code that runs outside any island.
