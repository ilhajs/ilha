...
import { error, redirect, router, wrapLayout, wrapError } from "./index";
...
const Home: Page = () => h("p", null, "home");

const About: Page = () => h("p", null, "about");
...
const FailingMissing: Page = () => {
  error(404, "missing page");
};

const missingBoundary = (err: { message: string }) =>
  h("p", null, `boundary:${err.message}`);

const FailingTeapot: Page = () => {
...
const Missing: Page = () => h("p", null, "not-found-page");

const FailingGone: Page = () => {
  error(404, "gone");
};
...
describe("router.renderResponse", () => {
  it("renders a matched page as html with status 200", async () => {
    ...
    expect(res.html).toContain("about");
    expect(res.html).toContain("data-ilha");
  ...
  it("collects head entries declared by the page", async () => {
    ...
    const headTags = res.head?.headTags ?? "";
  ...
  it("wraps pages in layouts with children", async () => {
    ...
    expect(res.html).toContain('class="shell"');
    expect(res.html).toContain("home");
  });

  it("routes through the error boundary on RouteError", async () => {
    const r = router()
      .route("/missing", FailingMissing)
      .errorBoundary("/missing", missingBoundary);
    ...
    try {
      ...
      expect(res.html).toContain("boundary:missing page");
    } finally {
  ...
  it("wrapError converts page throws into the handler view", async () => {
    const r = router().route(
      "/teapot",
      wrapError(({ message }) => h("p", null, message), FailingTeapot)
    );
    ...
    expect(res.html).toContain("short and stout");
  });
  ...
  it("renders notFound for unmatched paths", async () => {
    ...
    expect(res.html).toContain("not-found-page");
  });
...
describe("router.respond", () => {
  ...
  it("surfaces error statuses on the Response", async () => {
    ...
    console.error = () => {};
    try {
      const r = router().route("/gone", FailingGone);
