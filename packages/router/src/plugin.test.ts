...
const plugin = (options: Record<string, string> = {}): VitePagesPlugin => {
  // SAFETY: vite() returns a Plugin; tests only call the hooks declared above.
  const p = ilhaPages.vite(options) as VitePagesPlugin;
  return p;
};

const fileRef = (s: string | undefined) =>
  s?.match(/from ['"](?<spec>.+)['"]/u)?.groups?.spec;
...
describe("pages — plugin", () => {
  ...
  it("load for ilha:loaders is an empty side-effect module", () => {
    const p = plugin();
    expect(p.load("\0ilha:loaders")).toBe(
  ...
  it("load for ilha:pages/client re-exports pageRouter and registry", async () => {
    ...
    const clientSpec = resolveGeneratedPaths(outDir).clientFile.replace(
      /\.ts$/u,
      ""
    );
...
describe("pages — ?client virtual module", () => {
  ...
  it("resolveId without ?client suffix is unaffected", () => {
    const p = plugin();
    expect(
      p.resolveId("./foo.ts", "/proj/.ilha/pages.client.ts")
    ).toBeUndefined();
  });
  ...
  it("load(?client) result does not re-export non-default symbols", () => {
    ...
    expect(result).toMatch(/export\s*\{\s*default\s*\}/u);
